...
import { UIRouterLit } from './core';
import { UIRouterLitElement } from './ui-router';
import {
  UiSrefElement,
  UiSrefTargetEvent,
...
export class UiSrefActiveDirective extends AsyncDirective {
  ...
  async update(
    part: ElementPart,
    [{ activeClasses, exactClasses, state, params = {}, options = {}, targetStates }]: [UiSrefActiveParams]
  ) {
  ...
  async firstUpdated({ targetStates }: UiSrefActiveParams) {
    ...
    this._deregisterOnStatesChanged = this.uiRouter.stateRegistry.onStatesChanged(this.onStatesChanged) as deregisterFn;
