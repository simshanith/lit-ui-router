...
import { LitViewConfig, UIRouterLit } from './core';
import { UIRouterLitElement, UiRouterContextEvent } from './ui-router';
...
export class UiView extends LitElement {
  ...
  @state()
  private component: RoutedLitTemplate;
  ...
  private _deregisterUIView: deregisterFn;
  private _deregisterUiCanExitHook: deregisterFn;
  private _deregisterUiOnParamsChangedHook: deregisterFn;
  ...
  private async _applyUpdatedConfig(config: LitViewConfig) {
    ...
    this.component = (config.viewDecl as NormalizedLitViewDeclaration).component;
    this.requestUpdate();
  }
  ...
  private setupUiView() {
    ...
    this.disconnectedHandlers.push(
      router.transitionService.onSuccess(
        {},
        (trans) => this._invokeUiOnParamsChangedHook(trans)
      ) as deregisterFn
    );

    this.disconnectedHandlers.push(
      router.viewService.registerUIView(
        this._uiViewData
      )
    );
  }
  ...
  private _invokeUiOnParamsChangedHook($transition$: Transition) {
    ...
    if (isFunction(uiOnParamsChanged)) {
      ...
      const toParams: { [paramName: string]: unknown } = $transition$.params('to');
      const fromParams: { [paramName: string]: unknown } = $transition$.params(
        'from'
      );
