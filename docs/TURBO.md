...
| `tools/repo-checks`                                       | Adds `check:patches`, `check:graph-edges`, `check:task-inputs`, `check:workspace-deps`, `check:knip` (config in `tools/repo-checks/knip.jsonc`), each keyed on repo-wide `$TURBO_ROOT$` globs; over-approximated `test` inputs      |
