...
| <a href="/app-mobx" target="_self">`/app-mobx`</a>                   | 3+4 · flagship | The same app and the same verdicts, with [MobX bindings](./guides/mobx) for state — routing is identical, so this varies the client-state axis, not a routing one.                                                               |
