...
| [`eslint-plugin-lit-ui-router`](https://npmx.dev/package/eslint-plugin-lit-ui-router)                   | ESLint rules that understand the router's directives — a directive-aware `anchor-is-valid` (release candidate). [Guide →](./packages/eslint-plugin) |
