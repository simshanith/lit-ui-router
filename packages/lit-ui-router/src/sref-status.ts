...
export class SrefTargets {
  ...
  onLink(event: UiSrefTargetEvent): void {
    this.links.set(srefEventLink(event), event.detail.targetState);
  }

  /** Forgets a link whose part left the DOM; false when it was not ours. */
  onLinkRemoved(event: Event): boolean {
    return this.links.delete(srefEventLink(event));
  }
