...
| `docs/worker/index.ts` | [Worker script](https://developers.cloudflare.com/workers/static-assets/routing/worker-script/) - SPA fallback for `/app/*` and `/app-mobx/*` deep links that match the sample apps' route patterns (shared from `sample-app-routes`); everything else serves static assets, with misses falling back to VitePress's `404.html` |
