...
// cite — the latest version, its publish date and the full dist-tag map —
// fetched by script, never by hand.  A package whose `rc` tag is ahead of
// `latest` is read off `tags`; `version` stays the `latest` tag.  Universe:
// the @uirouter family constellation (sheet 4's spine) plus this repo's
// published packages, read from the master snapshot's member table.  Writes
// www/atlas.lit-ui-router.dev/data/census-npm.json.
...
for (const name of names) {
  ...
  const raw = JSON.parse(execFileSync('npm', ['view', name, 'version', 'time', 'dist-tags', '--json']).toString('utf8'));
...
writeData('census-npm.json', {
  ...
  wasGeneratedBy: 'www/atlas.lit-ui-router.dev/generator/census-npm.mjs',
  used: 'npm view <pkg> version time dist-tags (registry.npmjs.org)',
