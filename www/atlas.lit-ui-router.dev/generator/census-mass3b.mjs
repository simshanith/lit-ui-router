...
// www/atlas.lit-ui-router.dev/data/census-mass3b.json (rows + real task list, PROV-O meta).
...
writeData('census-mass3b.json', {
  ...
  wasGeneratedBy: 'www/atlas.lit-ui-router.dev/generator/census-mass3b.mjs',
  used: `git archive ${basis.ref} @ ${basis.sha} + pnpm install --frozen-lockfile --silent (mise-provisioned pnpm)`,
  wasAssociatedWith: [`turbo ${graph.turboVersion}`, 'scc 4.0.0 (mise x aqua:boyter/scc)', 'pnpm (mise-provisioned)'],
  drift,
  rows,
  tasks,
}, ['drift', 'rows', 'tasks']);
