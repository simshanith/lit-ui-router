...
const LIT_PEERS = declaredBy('lit');
const SSR_LIT = contract('lit-ui-router-ssr', 'lit-ui-router');
const SSR_SRV = contract('lit-ui-router-ssr', 'ui-router-server');
const MBX = declares('lit-ui-router-mobx');
const SHIPPED = C.rows.filter((r) => r.kind === 'dep');

export const sheet2b = {
  num: '2B', id: 'coupling-bench', rev: 'D',
  title: 'THE COUPLING BENCH',
  sub: `ALTITUDE 2 — INTERACTIVE PLATE: sheet 2A’s joints made live · ${T.nodes} nodes in four columns, every tie a straight run · each of the ${T.drawnContracts} drawn edges is a published contract, with its declared range under the pointer`,
  scale: `${T.nodes} NODES · ${T.drawnContracts} DRAWN CONTRACTS`,
  form: 'INTERACTIVE COUPLING GRAPH',
  caption: `The same assembly as sheets 2 and 2A, reduced to what a consumer's installer actually reads: ${T.nodes} buildings and ${T.drawnContracts} contracts. Hover an edge for the range it declares and the section it lives in; hover a building for its version, what it declares, and what declares it.`,
};

// the cover index's fit verdict, told from the plate's own tallies
export const SHEET2B_VERDICT = `2A’s joints made live — every one of the ${T.drawnContracts} drawn edges is a published contract with its declared range under the pointer: ${T.peers} peers to ${T.deps} dependencies across the ${T.published} packages, and neither dependency is a router`;

const notes = `
<p><strong>This plate is sheet 2A with the joints made live.</strong> Sheet 2 explodes the assembly and sheet 2A draws each coupling at reading size; both letter the API call that makes the joint. This one drops the API entirely and draws the other contract — the one <code>npm install</code> reads. Its arrangement is deliberately 2A’s: <code>@uirouter/core</code> as the socket wall at the left with <code>lit</code> above it, the lit companions in a column at the right in 2A’s order, the server below them with the coupling that exists only to be crossed out, and beyond that column the two companions that tie the flagship from outside it.</p>
<p><strong>The middle column is an argument, not a spacer.</strong> ${LIT_PEERS.length} of the ${T.published} published packages declare <code>lit</code> outright; <code>ui-router-navigation-location-plugin</code> needs neither it nor anything that does, and the bench says so by standing it in a column of its own between the wall and the lit column, on the wall’s own baseline. Every other building on this bench reaches left across two column gaps; this one reaches across one, along a straight horizontal run, and that run is the whole of its coupling. With that column standing clear, nothing sits between <code>lit-ui-router-mobx</code> and <code>lit-ui-router</code> either, so their tie is a plain vertical. Every line on the plate is straight by construction: the generator <em>throws</em> if a same-column tie is laid through a third building, so a curve drawn around a neighbour is a build error rather than a layout choice.</p>
<p><strong>An edge is one declaration, not a relationship.</strong> Every line is a single entry in a published <code>package.json</code> — its section (<code>peerDependencies</code> or <code>dependencies</code>), its declared range, and whether <code>peerDependenciesMeta</code> marks it optional. <code>devDependencies</code> are not drawn: they bind this workspace and never reach a consumer’s install. Of ${T.contracts} contracts the ${T.published} published packages declare, ${T.drawnContracts} land on a node that is on this bench; the other ${T.contracts - T.drawnContracts} are filed on the plate and read out in the panel under OFF THE BENCH — <code>effect</code>, <code>@lit-labs/ssr</code>, <code>eslint</code> and the rest, contracts on things this repo neither publishes nor draws.</p>
<p><strong>One node ties two siblings, and it is the reason the bench grew a column.</strong> <code>lit-ui-router-ssr</code> declares <code>lit-ui-router ${SSR_LIT.range}</code> <em>and</em> <code>ui-router-server ${SSR_SRV.range}</code> — the only building here whose ties reach two of its own siblings. It cannot stand in the companions column: a tie laid down that column to the flagship would run straight through <code>lit-ui-router-mobx</code>, and the generator throws on exactly that rather than bowing the line around it. So it stands one column further out with <code>lit-ui-router-effect</code>, whose single sibling tie — <code>lit-ui-router ${contract('lit-ui-router-effect', 'lit-ui-router').range}</code> — has the same shape as mobx’s and the same problem.</p>
...
<p><strong>Counting rules.</strong> ${CORE_PEERS.length} of the ${T.published} published packages name <code>@uirouter/core</code>; ${CORE_PEERS.filter((r) => r.optional).length} of those name it optionally. <code>${NAV_CORE.from}</code> declares exactly one contract in the world — <code>@uirouter/core ${NAV_CORE.range}</code> — which is the plate’s own way of saying it has never heard of Lit. Massing and storeys are the brick schedule’s: front-face area tracks sloc, the smallest companions are clamped up to a legible minimum, and a building’s storeys are its <code>courses</code> band from <code>census-bricks.json</code>, never a pipeline tier.</p>`;
