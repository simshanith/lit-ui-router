...
const basisOf = (s, plate) => {
  ...
  if (used.startsWith('www/atlas.lit-ui-router.dev/data/')) return /\bhistoryLog\(/.test(s) ? 'master plate + git log' : 'master plate';
...
for (const file of CENSUS) {
  ...
  if (!PLATE_META.has(plate)) throw new Error(`census-atlas: ${file} writes ${plate}, which is not filed in www/atlas.lit-ui-router.dev/data/`);
...
for (const d of drawings) {
  for (const r of d.reads) {
    if (!PLATE_META.has(r)) throw new Error(`census-atlas: ${d.file} reads ${r}, which is not filed in www/atlas.lit-ui-router.dev/data/`);
  }
}
