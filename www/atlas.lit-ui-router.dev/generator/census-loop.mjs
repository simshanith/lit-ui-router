...
try {
  ...
  const STATIONS = [
    ...
    { id: 'hall', label: 'TRANSITION HALL', sub: 'onBefore | onStart · resolve | onSuccess', file: 'ui-view.ts', line: 311, expect: 'router.transitionService.onBefore({}, (trans) => {' },
    { id: 'view', label: '<ui-view>', sub: 'the viewport', file: 'ui-view.ts', line: 79, expect: 'export class UiView extends LitElement {' },
    { id: 'child', label: "<ui-view name='detail'>", sub: 'nested — stands on its parent', file: 'ui-view.ts', line: 189, expect: 'static seekParentView(candidate: Element): UiView | null {' },
    { id: 'render', label: 'LIT RENDER', sub: 'RoutedLitTemplate · html``', file: 'ui-view.ts', line: 460, expect: 'render(): Node | TemplateResult {' },
    { id: 'document', label: 'THE DOCUMENT', sub: 'window → body → <ui-router> → …', file: 'ui-router.ts', line: 48, expect: 'export class UIRouterLitElement extends LitElement {' },
    { id: 'link', label: '<a uiSref>', sub: 'the topmost plate', file: 'ui-sref.ts', line: 120, expect: 'export class UiSrefDirective extends AsyncDirective {' },
    { id: 'active', label: 'uiSrefActive', sub: 'perch — toggles .active below', file: 'ui-sref-active.ts', line: 217, expect: 'export class UiSrefActiveDirective extends AsyncDirective {' },
  ...
  const LEGS = [
    ...
    { id: 'run', from: 'core', to: 'hall', kind: 'loop', carrier: 'transitionService.onBefore / onStart / onSuccess', short: 'match → run',
      ev: { file: 'ui-view.ts', line: 311, expect: 'router.transitionService.onBefore({}, (trans) => {' } },
    { id: 'activate', from: 'hall', to: 'view', kind: 'loop', carrier: 'viewService.registerUIView → configUpdated', short: 'viewconfigs activate',
      ev: { file: 'ui-view.ts', line: 323, expect: 'router.viewService.registerUIView(this._uiViewData),' } },
    { id: 'descend', from: 'view', to: 'child', kind: 'loop', carrier: 'fqn = parentFqn + "." + name', short: 'one level down',
      ev: { file: 'ui-view.ts', line: 294, expect: "const fqn = parentFqn ? parentFqn + '.' + name : name;" } },
    { id: 'render', from: 'view', to: 'render', kind: 'loop', carrier: 'requestUpdate() → render() → component({ router, resolves, transition })', short: 'state.component / template',
      ev: { file: 'ui-view.ts', line: 479, expect: 'const props: UIViewInjectedProps = { router, resolves, transition };', span: 3 } },
    { id: 'commit', from: 'render', to: 'document', kind: 'loop', carrier: 'createRenderRoot(): this — the template lands in the light DOM', short: 'directives commit',
      ev: { file: 'ui-view.ts', line: 102, expect: 'createRenderRoot(): this {', span: 3 } },
    { id: 'click', from: 'link', to: 'core', kind: 'click', carrier: '$state.go(state, params, options)', short: 'click → go()',
      ev: { file: 'ui-sref.ts', line: 332, expect: 'void $state.go(state, params, options);', span: 2 } },
    { id: 'ctx', from: 'view', to: 'document', kind: 'event', carrier: 'ui-router-context (composed) — the shell answers with its router', short: 'ui-router-context',
      ev: { file: 'ui-router.ts', line: 92, expect: 'candidate.dispatchEvent(uiRouterContextEvent);', span: 2 } },
    { id: 'nest', from: 'child', to: 'view', kind: 'event', carrier: 'ui-view-context (composed) — the parent adopts', short: 'ui-view-context',
      ev: { file: 'ui-view.ts', line: 191, expect: 'candidate.dispatchEvent(uiViewContextEvent);', span: 2 } },
    { id: 'target', from: 'link', to: 'active', kind: 'event', carrier: 'uiSrefTarget (bubbles, composed)', short: 'uiSrefTarget bubbles',
      ev: { file: 'ui-sref.ts', line: 224, expect: 'this.element.dispatchEvent(uiSrefTargetEvent(this.targetState));' } },
    { id: 'tap-active', from: 'hall', to: 'active', kind: 'tap', carrier: 'transitionService.onStart({}, onTransitionStart) → transitionStateChange', short: 'onStart tap',
      ev: { file: 'ui-sref-active.ts', line: 550, expect: 'this._deregisterOnStart = this.uiRouter.transitionService.onStart(', span: 3 } },
  ...
  const WALK = [
    { title: 'AT REST — /people', legs: ['ctx', 'nest', 'target'],
      ...
      ev: [{ file: 'ui-view.ts', line: 209, expect: 'this.uiRouter = UIRouterLitElement.seekRouter(this)!;' },
        { file: 'ui-view.ts', line: 196, expect: 'this.parentView = this.constructor.seekParentView(this)!;' },
        { file: 'ui-sref-active.ts', line: 639, expect: 'this.targetStates.add(targetState);', span: 2 }] },
    { title: 'THE CLICK', legs: ['click'],
      ...
      ev: [{ file: 'ui-sref.ts', line: 316, expect: 'onClick = (event: MouseEvent): void => {' },
        { file: 'sref-internals.ts', line: 194, expect: 'isNativeLink(element) &&', span: 3 },
        { file: 'ui-sref.ts', line: 332, expect: 'void $state.go(state, params, options);', span: 2 }] },
    { title: 'onBefore — CAN WE LEAVE?', legs: ['run'],
      ...
      ev: [{ file: 'ui-view.ts', line: 311, expect: 'router.transitionService.onBefore({}, (trans) => {', span: 3 },
        { file: 'ui-view.ts', line: 353, expect: 'if (trans.exiting().includes(state)) {', span: 3 }] },
    { title: 'onStart — THE PERCH HEARS IT', legs: ['run', 'tap-active'],
      ...
      ev: [{ file: 'ui-sref-active.ts', line: 676, expect: 'onTransitionStart = (trans: Transition): void => {', span: 3 },
        { file: 'ui-sref-active.ts', line: 654, expect: 'const { active, exact, entering, exiting } = status;', span: 3 }] },
    { title: 'RESOLVE', legs: ['run'],
      ...
      ev: [{ file: 'ui-view.ts', line: 468, expect: 'const resolvables = this.resolveContext', span: 3 },
        { file: 'ui-view.ts', line: 472, expect: '.filter((r) => r.resolved);' }] },
    { title: 'VIEWS ACTIVATE', legs: ['activate'],
      ...
      ev: [{ file: 'core.ts', line: 239, expect: "this.viewService._pluginapi._viewConfigFactory('lit', viewConfigFactory);" },
        { file: 'ui-view.ts', line: 302, expect: 'configUpdated: this._viewConfigUpdated.bind(this),' },
        { file: 'ui-view.ts', line: 142, expect: 'this.resolveContext = new ResolveContext(config.path);', span: 3 }] },
    { title: 'ONE LEVEL DOWN', legs: ['descend'],
      ...
      ev: [{ file: 'ui-view.ts', line: 294, expect: "const fqn = parentFqn ? parentFqn + '.' + name : name;" },
        { file: 'ui-view.ts', line: 323, expect: 'router.viewService.registerUIView(this._uiViewData),' }] },
    { title: 'LIT RENDER', legs: ['render'],
      ...
      ev: [{ file: 'ui-view.ts', line: 362, expect: 'requestUpdate(...args: Parameters<LitElement[\'requestUpdate\']>): void {', span: 3 },
        { file: 'ui-view.ts', line: 474, expect: 'const resolves = resolvables', span: 3 },
        { file: 'ui-view.ts', line: 479, expect: 'const props: UIViewInjectedProps = { router, resolves, transition };', span: 3 }] },
    { title: 'THE DOCUMENT', legs: ['commit', 'target'],
      ...
      ev: [{ file: 'ui-view.ts', line: 102, expect: 'createRenderRoot(): this {', span: 3 },
        { file: 'ui-sref.ts', line: 208, expect: "this.element.setAttribute('href', this.href);" },
        { file: 'ui-sref.ts', line: 223, expect: 'if (targetChanged) {', span: 2 }] },
    { title: 'onSuccess — THE VIEW HEARS IT', legs: ['run', 'activate'],
      ...
      ev: [{ file: 'ui-view.ts', line: 317, expect: 'router.transitionService.onSuccess({}, (trans) =>', span: 2 },
        { file: 'ui-view.ts', line: 373, expect: 'private _invokeUiOnParamsChangedHook($transition$: Transition) {', span: 2 },
        { file: 'ui-view.ts', line: 426, expect: 'instance.uiOnParamsChanged(newValues, $transition$);' }] },
    { title: 'THE PERCH AND THE SKYBRIDGE', legs: ['tap-active', 'tap-controller'],
      ...
      ev: [{ file: 'ui-sref-active.ts', line: 688, expect: 'trans.promise.then(', span: 3 },
        { file: 'ui-sref-active.ts', line: 333, expect: 'this.element!.classList.add(className);' },
