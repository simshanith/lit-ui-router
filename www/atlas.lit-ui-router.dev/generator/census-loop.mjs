...
try {
  ...
  const STATIONS = [
    ...
    { id: 'document', label: 'THE DOCUMENT', sub: 'window → body → <ui-router> → …', file: 'ui-router.ts', line: 33, expect: 'export class UIRouterLitElement extends LitElement {' },
    { id: 'link', label: '<a uiSref>', sub: 'the topmost plate', file: 'ui-sref.ts', line: 249, expect: 'export class UiSrefDirective extends AsyncDirective {' },
    { id: 'active', label: 'uiSrefActive', sub: 'perch — toggles .active below', file: 'ui-sref-active.ts', line: 321, expect: 'export class UiSrefActiveDirective extends AsyncDirective {' },
  ...
  const LEGS = [
    ...
    { id: 'click', from: 'link', to: 'core', kind: 'click', carrier: '$state.go(state, params, options)', short: 'click → go()',
      ev: { file: 'ui-sref.ts', line: 461, expect: 'void $state.go(state, params, options);', span: 2 } },
    { id: 'ctx', from: 'view', to: 'document', kind: 'event', carrier: 'ui-router-context (composed) — the shell answers with its router', short: 'ui-router-context',
      ev: { file: 'ui-router.ts', line: 72, expect: 'candidate.dispatchEvent(uiRouterContextEvent);', span: 2 } },
    ...
    { id: 'target', from: 'link', to: 'active', kind: 'event', carrier: 'uiSrefTarget (bubbles, composed)', short: 'uiSrefTarget bubbles',
      ev: { file: 'ui-sref.ts', line: 353, expect: 'this.element.dispatchEvent(uiSrefTargetEvent(this.targetState));' } },
    { id: 'tap-active', from: 'hall', to: 'active', kind: 'tap', carrier: 'transitionService.onStart({}, onTransitionStart) → transitionStateChange', short: 'onStart tap',
      ev: { file: 'ui-sref-active.ts', line: 654, expect: 'this._deregisterOnStart = this.uiRouter.transitionService.onStart(', span: 3 } },
  ...
  const WALK = [
    { title: 'AT REST — /people', legs: ['ctx', 'nest', 'target'],
      ...
      ev: [{ file: 'ui-view.ts', line: 209, expect: 'this.uiRouter = UIRouterLitElement.seekRouter(this)!;' },
        ...
        { file: 'ui-sref-active.ts', line: 743, expect: 'this.targetStates.add(targetState);', span: 2 }] },
    { title: 'THE CLICK', legs: ['click'],
      ...
      ev: [{ file: 'ui-sref.ts', line: 445, expect: 'onClick = (event: MouseEvent): void => {' },
        { file: 'ui-sref.ts', line: 230, expect: 'isNativeLink(element) &&', span: 3 },
        { file: 'ui-sref.ts', line: 461, expect: 'void $state.go(state, params, options);', span: 2 }] },
    ...
    { title: 'onStart — THE PERCH HEARS IT', legs: ['run', 'tap-active'],
      ...
      ev: [{ file: 'ui-sref-active.ts', line: 780, expect: 'onTransitionStart = (trans: Transition): void => {', span: 3 },
        { file: 'ui-sref-active.ts', line: 758, expect: 'const { active, exact, entering, exiting } = status;', span: 3 }] },
    ...
    { title: 'THE DOCUMENT', legs: ['commit', 'target'],
      ...
      ev: [{ file: 'ui-view.ts', line: 102, expect: 'createRenderRoot(): this {', span: 3 },
        { file: 'ui-sref.ts', line: 337, expect: "this.element.setAttribute('href', this.href);" },
        { file: 'ui-sref.ts', line: 352, expect: 'if (targetChanged) {', span: 2 }] },
    ...
    { title: 'THE PERCH AND THE SKYBRIDGE', legs: ['tap-active', 'tap-controller'],
      ...
      ev: [{ file: 'ui-sref-active.ts', line: 792, expect: 'trans.promise.then(', span: 3 },
        { file: 'ui-sref-active.ts', line: 437, expect: 'this.element!.classList.add(className);' },
