...
// www/atlas.lit-ui-router.dev/generator/ and www/atlas.lit-ui-router.dev/data/ at build time: probes are the
...
for (const r of A.rows) {
  if (!BASIS_SHORT.has(r.basis)) throw new Error(`appendix A2: basis "${r.basis}" (${r.writer}) has no short label`);
}
...
${txt(SX + 28, HARNESS_Y + 26, 'mise-provisioned pnpm --frozen-lockfile → .bin/turbo', 'lbls')}
...
const ARIA = `The census pipeline drawn as a left-to-right flow of dataflow, not containment. `
  ...
  + `${S.byTier.T1} pure-tree, ${S.byTier.T2} history, ${S.byTier.T3} execution, the last fed by an install harness running mise-provisioned pnpm with a frozen lockfile and the tree's own turbo binary. `
...
${txt(30, 62, `one archive · ${S.probes} probe stations · ${S.plates} filed plates · ${S.drawings} drawings that read them — introspected from www/atlas.lit-ui-router.dev/generator/ at build time`, 'lbls')}
...
export const sheetA2 = {
  num: 'A2', id: 'pipeline', rev: 'D',
  // Appendix chrome: the office measures the atlas, so it stands at no altitude.
  head: 'APPENDIX A2 · META',
  appendix: true,
  title: 'THE SURVEY OFFICE',
  sub: `APPENDIX · META — the atlas measuring itself · ${S.probes} probes, ${S.plates} plates, ${S.drawings} drawings · every station, plate and edge on this plate introspected from www/atlas.lit-ui-router.dev/generator/ at build time · all plates pinned to ${BASIS}`,
...
<p><strong>Why a flow graph and not a city.</strong> Sheet 3 refuses to draw a task manager as a place, and the same refusal applies here. A pipeline's truth is dataflow — ref to archive to measurement to plate to drawing — and the lego/containment vocabulary the atlas uses elsewhere would encode a claim about <em>what contains what</em>, which is exactly the wrong claim about a pipeline. The vocabulary that fits is the one the atlas already owns: probes as survey instruments, snapshots as filed plates, sheets as finished drawings. Tier is station type; the cabinet is a cabinet; the rack is a rack. The node and edge arrays behind this drawing (${S.nodes} nodes over ${WORD[Object.keys(S.byKind).length]} kinds, ${S.edges} edges over writes, reads and imports) are also the data model the interactive lane mounts — the picture is a query, not a hand-drawing.</p>`,
  key: [
    ...
    keyRow('<rect x="4" y="3" width="40" height="12" class="sk fp" rx="4"/>', 'a filed plate — one drawer of www/atlas.lit-ui-router.dev/data/'),
