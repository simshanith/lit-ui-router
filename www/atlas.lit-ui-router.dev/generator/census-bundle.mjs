...
// build), never the working tree.  Writes www/atlas.lit-ui-router.dev/data/census-bundle.json.
//
...
writeData('census-bundle.json', {
  ...
  wasGeneratedBy: 'www/atlas.lit-ui-router.dev/generator/census-bundle.mjs',
  used: `git archive ${basis.ref} @ ${basis.sha} + pnpm install --frozen-lockfile --silent (mise-provisioned pnpm) + turbo run build + vite build (JS API)`,
  wasAssociatedWith: ['pnpm (mise-provisioned)', 'turbo', `vite ${vite.version}`],
  app: APP_DIR,
