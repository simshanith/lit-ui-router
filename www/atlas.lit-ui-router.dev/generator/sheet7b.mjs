...
//         www/atlas.lit-ui-router.dev/data/census-shadow.json: lit share = extent% × line% (how much
...
//         www/atlas.lit-ui-router.dev/data/census-plate.json, last run green 2026-08-17 (all cache
...
const cityOf = (dir) => {
  ...
  if (!r) throw new Error(`plate 7B: member ${dir} is missing from www/atlas.lit-ui-router.dev/data/census-city.json`);
  return r;
};
const steamOf = (dir) => {
  const c = STEAM_ROW.get(dir);
  if (c === undefined) throw new Error(`plate 7B: member ${dir} is missing from www/atlas.lit-ui-router.dev/data/census-steam.json`);
  return c;
};
...
const lampOf = (dir) => {
  ...
  if (!s) throw new Error(`plate 7B: member ${dir} is missing from www/atlas.lit-ui-router.dev/data/census-shadow.json`);
...
${txt(108, 742, 'atlas’s own www/atlas.lit-ui-router.dev/generator among them, so no plant carries a triangle.', 'lblf')}
${txt(108, 756, 'the struck triangle at the left is this channel’s legend, not an alarm.', 'lbls')}
${txt(108, 776, `checked 2026-08-31 · oxlint over www/atlas.lit-ui-router.dev/generator, exit 0 · build graph ${BUILD.real} real`, 'lblf')}`;
