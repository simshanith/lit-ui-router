...
const INSTRUMENTS = [
  ...
  ['lint & probe fleet', [/^tools\/lit-template-lint\//, /^tools\/lint-elements\//, /^tools\/warn-lanes\//, /^tools\/eslint\//, /^tools\/eslint-ts-parser\//, /^tools\/embed-heights\//, /^tools\/bundle-probe\//, /^tools\/vue-check\//, /^tools\/lcov-rebase\//, /^tools\/build_and_test\//]],
  ['repo checks — graph edges, task inputs, knip, patches', [/^tools\/repo-checks\//]],
  ['bootstrap — pre-install manifest reads', [/^tools\/bootstrap\//]],
...
writeData('census-yard.json', {
  ...provenance(snap, 'www/atlas.lit-ui-router.dev/generator/census-yard.mjs'),
  orphans,
