...
//   footprints + annexes — www/atlas.lit-ui-router.dev/data/census-city.json (sheet 7's own plate)
//   cat, extent, line/branch/func, the lit footprint — www/atlas.lit-ui-router.dev/data/census-shadow.json,
...
const cityOf = (dir) => {
  ...
  if (!r) throw new Error(`plate 7A: member ${dir} is missing from www/atlas.lit-ui-router.dev/data/census-city.json`);
  return r;
};
const shadowOf = (dir) => {
  const r = SH_ROW.get(dir);
  if (!r) throw new Error(`plate 7A: member ${dir} is missing from www/atlas.lit-ui-router.dev/data/census-shadow.json`);
  return r;
};
