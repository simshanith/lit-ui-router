...
// www/atlas.lit-ui-router.dev/data/census-plate.json, the checked-in snapshot census-plate.mjs
...
const pipe = (n) => {
  ...
  if (!p) throw new Error(`sheet 12: pipeline ${n} is missing from www/atlas.lit-ui-router.dev/data/census-plate.json`);
  return p;
};
const CI = pipe('ci');
...
const nm = (n) => {
  const v = NAMES.get(n);
  if (!v) throw new Error(`sheet 12: task name ${n} is missing from the ci graph in www/atlas.lit-ui-router.dev/data/census-plate.json`);
  return v;
};
...
if (!TURBO) throw new Error('sheet 12: www/atlas.lit-ui-router.dev/data/census-plate.json carries no turbo version in wasAssociatedWith');
...
const CHAIN = [
  ...
  if (!k) throw new Error(`sheet 12: chain node ${id} is not in the ci graph in www/atlas.lit-ui-router.dev/data/census-plate.json`);
  return [id, k === 'r' ? 1 : 0];
});
...
export const sheet12 = {
  ...
  sub: `ALTITUDE 3¼ — the monorepo as its CI reads it · every task node punched · ${BASIS}`,
  scale: 'PR CI GRAPH',
  form: 'REGISTER PLATE',
  svg,
