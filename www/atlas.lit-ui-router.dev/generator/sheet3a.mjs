...
const H = (file) => {
  ...
  if (!r) throw new Error(`plate 3A: mise home ${file} is missing from www/atlas.lit-ui-router.dev/data/census-handoff.json`);
  return r;
};
const pipe = (n) => {
  const p = PLATE.pipelines[n];
  if (!p) throw new Error(`plate 3A: pipeline ${n} is missing from www/atlas.lit-ui-router.dev/data/census-plate.json`);
  return p;
};
const CI = pipe('ci');
