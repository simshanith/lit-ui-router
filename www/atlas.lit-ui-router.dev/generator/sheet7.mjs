...
// ---- census: every number below comes from www/atlas.lit-ui-router.dev/data/census-city.json ----
...
export const PLACED = [
  ...
  // moved east off 200 when lit-ui-router's annex grew into its west wall (#833 refresh)
  [2,  'ui-router-server',         'packages/ui-router-server',          'pkg',  'line',   222,  20, `the server adapter · annex ${ratio('packages/ui-router-server')}×`],
  ...
  // --- born 2026-09-04 (#703) ------------------------------------------------------
  // moved off the 430 row when the hygiene block below took the yard's east end:
  // its old lot left 10.9 units to build_and_test's annex, the sheet's tightest pair
  [32, '@tools/embed-heights',     'tools/embed-heights',                'tool', 'report', 449, 462, 'docs embed heights, measured in Chromium — outside ci'],
  ...
  [35, '@tools/repo-checks',       'tools/repo-checks',                  'tool', 'pr',     449, 385, 'graph edges, task inputs, knip and patches — four checks the root lint task waits on'],
  ...
  // sheet 13's reading box, so the two plates keep the same coordinate
  [37, 'lit-ui-router-effect',     'packages/lit-ui-router-effect',      'pkg',  'line',    30, 210, `the effect bindings · private at this ref · annex ${ratio('packages/lit-ui-router-effect')}×`],
];

// [n, name, district, tier, x, y, srcFiles, srcSloc, specFiles, specSloc, note]
// the packages/ district's own size — the road register and the notes count it
// rather than naming a number that a new member falsifies
const PKG_N = PLACED.filter((p) => p[3] === 'pkg').length;
const WORDN = ['no', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight'];
...
// lit-ui-router-effect is a packages/ member but private at this ref, so the
// district's label counts members and publishes separately
const PUBLISHED_PKGS = M.filter((r) => r[2] === 'pkg').length - 1;
...
const roads = [
  ...
  //    turbo e2e dependsOn ^build, with docs#wrangler:dev.
...
// faces are opaque, so a front mass hides the rear edges of everything behind it.
...
${txt(1520, 76, `every number on this sheet is read from www/atlas.lit-ui-router.dev/data/census-city.json — ${BASIS}`, 'lblf', 'end')}
...
${txt(772, 123, `${DP.n} members, ${PUBLISHED_PKGS} published · ${DP.f} files · ${fmt(DP.sl)} sloc`, 'lblf')}
...
${txt(60, 585, `${M.filter((r) => r[2] === 'tool' && S(r[7]) === MIN).length} of its masses sit on this drawing’s minimum footprint`, 'lblf')}
...
${[
  ['A', 'builds into', 'packages/* → the sample apps · lit-ui-router → 4 of them, ui-router-server → routes'],
  ['B', 'builds into', `@tools/oxc-emit → all ${WORDN[PKG_N]} packages · devDep of each · runs build:js + build:types`],
  ['C', 'tests exercise', `@tools/dts-backtest → ${WORDN[PKG_N - 1]} of the ${WORDN[PKG_N]} · deps each · #test dependsOn ^build`],
  ['D', 'tests exercise', '@tools/lit-test-env + @tools/happy-dom → the annex · devDep of lit-ui-router + -mobx'],
  ['E', 'tests exercise', 'sample-app-lit-e2e → docs · workspace dep · turbo e2e dependsOn ^build, with docs#wrangler:dev'],
...
${txt(58, 798, 'apps → docs is road A continued: docs depends on three sample apps and on ui-router-server', 'lblf')}
...
export const sheet7 = {
  ...
  sub: `ALTITUDE 3½ — the same city as sheet 3, surveyed by mass · ${M.length} members in 4 districts, footprint ∝ √sloc, height ∝ authored files, spec mass drawn as an annex, gate severity in colour · ${BASIS}`,
  scale: 'WHOLE WORKSPACE',
  form: 'MEASURED CITY',
  svg,
  caption: 'Sheet 3 drew the monorepo as a process; this sheet counts who lives in it. Footprint ∝ √sloc, height ∝ authored files, tests drawn as annexes rather than deleted — and over that census the two things a census alone cannot say: which district each member belongs to, and which roads run between them. Every road is a workspace dependency or a turbo task edge, never an impression.',
  notes: `
