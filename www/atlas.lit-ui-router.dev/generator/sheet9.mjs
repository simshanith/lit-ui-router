...
// ---- census: every count comes from www/atlas.lit-ui-router.dev/data/census-shipped.json --------
...
// where it is FIRST claimed (vanilla → mobx → effect → hash) — the CDN ships it once, so
...
const PLAN = {
  ...
  'app: hash': [575, 398],
  'app: mobx': [655, 345],
  // the fourth shell, /app-effect (#721) — the lot east of vanilla, inside the group
  'app: effect': [660, 295],
};
...
// The top of the skyline is a race, and the order changes between refs — so the
// ranking is read off the plate rather than typed. RANKED is every district by
// gz, tallest first; nothing below names a district's position by hand.
const RANKED = [...all].sort((a, b) => b.gz - a.gz);
const rank = (name) => {
  const i = RANKED.findIndex((r) => r.name === name);
  if (i < 0) throw new Error(`sheet 9: no district named ${name} to rank`);
  return i + 1;
};
const ORD = ['first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eighth'];
const INTER_RANK = rank('inter fonts');
const TALLER_THAN_INTER = RANKED.slice(0, INTER_RANK - 1).map((r) => r.name);
const RUNNER_GAP = RANKED[0].gz - RANKED[1].gz;
// the cover index's fit verdict, told from the plate's own ranking
export const SHEET9_VERDICT = `the wire survey — the deploy's tallest district is ${RANKED[0].name} at ${KB(RANKED[0].gz)}, and the lettering alone outweighs every routed app the site exists to demonstrate (${KB(INTER.gz)} against ${KB(APPS)})`;
...
const svg = `<svg viewBox="0 0 1160 ${SY + 108 + half * 17}" role="img" aria-label="The production docs-site deploy drawn as an isometric city of ${all.length} districts: footprint area from file counts, height from gzipped bytes on the wire. The three tallest towers are ${RANKED.slice(0, 3).map((r) => r.name).join(', ')}. A vacant lot at the plan's edge marks the footprint of the demo corpora, which the site no longer ships. The four routed sample apps are small accent buildings totalling ${APP_PCT} percent of the deploy. There is no ghost block: the census walks backtick-quoted asset URLs too, and this deploy carries no unreachable files at all. A structure schedule lists every district with exact counts and its largest tenant.">
${defs(P)}
...
${txt(1120, 96, `${KB(INTER.gz)} of Inter — the lettering stands ${ORD[INTER_RANK - 1]}`, 'lbla', 'end')}
${txt(1120, 108, `on the wire; only ${TALLER_THAN_INTER.join(' and ')} stand taller`, 'lbla', 'end')}
...
${txt(110, 650, 'alike across all four', 'lbla')}
...
export const sheet9 = {
  ...
  notes: `
<p><strong>Method:</strong> every number on this sheet is read at build time from the checked-in plate <code>www/atlas.lit-ui-router.dev/data/census-shipped.json</code> — ${BASIS}. The probe builds <code>www/lit-ui-router.dev/dist</code> inside a materialized, installed archive of the ref, never the working tree, and measures it file by file: height is gzip level 9 of each file, the honest wire measure, since the CDN serves compressed. Footprint is file count, as on sheets 7 and 8. A reachability walk — every HTML shell and hashed chunk, following static asset references — sorts the assets into districts, and a shared chunk counts where it is first claimed (vanilla → mobx → effect → hash), because the CDN ships it once. This sheet extends the survey a step further: sheet 7 measures the source, sheet 8 what npm delivers, this sheet what one deploy actually ships — ${fmt(PLATE.totals.files)} files, ${MB(PLATE.totals.rawBytes)} on disk, ${MB(PLATE.totals.gzBytes)} on the wire. Sheet 10 goes one level in again and opens the bundle itself.</p>
<p><strong>The tallest building is the prose.</strong> The skyline is the site's ${PAGES.files} prerendered HTML pages at ${KB(PAGES.gz)}, with <code>${RANKED[1].name}</code> ${fmt(RUNNER_GAP)} bytes behind at ${KB(RANKED[1].gz)} — a race close enough that a few new guides, or one more demo, decide it. Inter's ${INTER.files} <code>woff2</code> faces stand ${ORD[INTER_RANK - 1]} at ${KB(INTER.gz)}: on the wire, this documentation site is still mostly prose and typography, and the one script district that outranks the lettering is the examples, led by a single galaxy. A vacant lot at the plan's edge marks where the demo corpora stood; they are no longer shipped.</p>
<p><strong>The product is a guest in its own city.</strong> The four routed sample apps — the thing the site exists to demonstrate — total ${KB(APPS)} gzipped, ${APP_PCT}% of the deploy. First claim seats <code>visualizer.esm</code> and the custom-elements manifest in <code>app: vanilla</code>, which is why that district reads ${VANILLA.files} files and ${KB(VANILLA.gz)} while the hash app's whole district is ${HASH.files} files and ${KB(HASH.gz)}. Two rules shape those numbers rather than the CDN: a scoped override holds the <code>@api-viewer</code>/<code>lit-dialog</code> stack to one lit major, so identical lit chunks hash identically <em>across</em> apps and part of mobx's download is chunks vanilla already ships; and the api-viewer docs panel, which renders only behind a feature flag, arrives in a lazy <code>api-docs</code> chunk rather than the eager main one. Same bytes on the CDN, fewer on the critical path.</p>
...
<p><strong>One example outweighs the router.</strong> The examples district — ${EXAMPLES.files} files, ${KB(EXAMPLES.gz)} — is led by the hellogalaxy demo's <code>model-viewer</code> chunk at ${KB(EXAMPLES.top.gz)} on its own, heavier than the vanilla, mobx and hash apps combined, delivered so one tutorial page can spin a galaxy.</p>`,
  key: [
