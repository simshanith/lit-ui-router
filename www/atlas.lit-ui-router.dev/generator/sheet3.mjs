...
// ---- census: every number below comes from www/atlas.lit-ui-router.dev/data/census-yard.json -------
...
// prefix (see www/atlas.lit-ui-router.dev/generator/census-yard.mjs).
...
// the inset's task-manager figures read the SAME plates sheet 3A draws from,
// so the two sheets cannot disagree about the machine
...
const M = [
  ...
  [7,  'tests — vitest harness',    'pr',      20,   215,    ...C('tests — vitest harness'), null, null, 'lit-test-env + happy-dom · 3 real browsers'],
...
${txt(58, SY + 58 + half * 17, `TOTAL — ${massed.length} massed structures · ${TOT_F} authored files · ${fmt(TOT_L)} sloc · ${COUNTED} · gate tier is colour, never height`, 'lbls')}`;
...
${ibox(254, 74, 166, 32, 'mise', `${HM.tasks} tasks · ${HM.withDepends} declare depends`)}
...
export const sheet3 = {
  ...
  sub: `ALTITUDE 3 — ${PUBLISHED} publishable packages · ${TOOLS} tools · ${HANDOFF.turbo.distinctNames} turbo task names · one packer, many readers · every mass ${COUNTED} from www/atlas.lit-ui-router.dev/data/census-yard.json on scc Code lines — ${TOT_F} authored files and ${fmt(TOT_L)} sloc across ${massed.length} massed structures, with the task-manager inset reading the same two plates sheet 3A draws`,
  scale: 'THE MONOREPO',
  form: 'ISOMETRIC CITY',
  svg,
  caption: 'The monorepo drawn as what it functionally is: a short conveyor that turns source into one tarball, inside a yard of instruments built to measure that tarball. Every structure carries its measured mass — footprint ∝ √sloc, height ∝ authored files — and gate severity is colour rather than height, because the two do not correlate: the blocks that stop the line are among the smallest on the sheet.',
  notes: `
<p><strong>Method — one basis, shared with sheet 4.</strong> Every massed structure was counted from this repo's authored source, ${COUNTED} and read from <code>www/atlas.lit-ui-router.dev/data/census-yard.json</code>: <code>.ts/.tsx/.js/.mjs</code> under each member's source directory, excluding <code>*.d.ts</code>, <code>*.test-d.ts</code>, <code>*.spec.*</code> and <code>*.test.*</code>, <code>specs/</code>, <code>__tests__/</code> and fixtures. <em>sloc</em> is <code>scc</code>'s <code>Code</code> count — string-aware, so the interior of a template literal counts as code rather than as one line. <code>@tools/release</code> hosts several instruments at once, so it is attributed by module prefix — <code>check-published-diff*</code> to structure 11, <code>check-exports*</code> to 10, the pack modules to 3, the release steps to 5 — with every file landing in exactly one structure and none counted twice. ${M.length} structures, ${massed.length} of them massed: ${TOT_F} authored files, ${fmt(TOT_L)} lines. One member is massed nowhere: <code>@tools/wintercg-globals</code> is a single <code>globals.d.ts</code>, and declaration files are outside this basis. The tarball, the registry and the boundary are diagrammatic.</p>
<p><strong>Mass is volume; volume does not predict authority.</strong> Footprint side is <code>1.6 · √sloc</code>, so plan area tracks lines; height is <code>1.5 px</code> per authored file, so a block's volume is its <code>sloc × files</code>. The two heaviest masses on the sheet are <em>material</em>: the sample apps (${nfl(18)}) and <code>packages/*/src</code> (${nfl(1)}). The ${PR_GATES.length} structures that can stop a pull request total ${PR_GATES.reduce((a, n) => a + nf(n), 0)} files between them, and one of them — <code>dts-backtest</code> — is a single ${nl(9)}-line <code>run.ts</code>. Encoding severity as height would imply the opposite; the census says a gate's authority has nothing to do with how much code it is.</p>
