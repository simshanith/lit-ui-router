...
export const sheetA1 = {
  ...
  // Appendix chrome: no altitude, no "OF n" — this plate is not in the ascent.
  head: 'APPENDIX A1 · META',
  // The one flag the shared chrome reads: no "OF n", and the title block's
  ...
  form: 'SPRITE STUDIES',
  svg,
  caption: 'Every other plate in this set measures the codebase. This one measures a decision: how a workspace member should look when a sheet draws it as a building. One demo member — 16 files, 1,900 sloc, massed under the same rules sheet 7 uses — is drawn fifteen times, five states along a wreck-to-splendor gauge under three competing treatments, and each treatment is priced. The verdict stands in the set: sheet 7B is the working plant, and sheet 13 is the weathering gauge those studies were commissioned for.',
  notes: `
<p><strong>Why an appendix, and not a sheet.</strong> The set's numbered sheets are ordered by <em>altitude</em> — how far back you stand from lit-ui-router — and every one of them cites a census plate. This plate stands nowhere on that ladder, because its subject is the atlas's own drawing convention rather than the codebase: it carries no <code>www/atlas.lit-ui-router.dev/data/*.json</code> import and no measured number. Filing it as A1 rather than as a numbered sheet keeps the ascent honest: thirteen altitudes, and behind them a folder about the atlas itself — this study of how its buildings are drawn, and appendix A2, the survey office that took its measurements. Its id is letter-prefixed for the same reason, so nothing that walks the set in ascent order picks it up by accident.</p>
...
<p><strong>Determinism.</strong> Study 1's vines are the only stochastic marks in the set, and they are seeded: a fixed linear-congruential stream per state, so the same plate is drawn byte-for-byte on every build. Nothing here is animated, nothing is rotated, and the fifteen blocks share one projection helper with <code>generator/helpers.mjs</code>'s <code>iso()</code>.</p>`,
  key: [
    ...
    keyRow('<rect x="6" y="6" width="7" height="6" class="skg fg"/><rect x="16" y="6" width="7" height="6" class="skf fp2"/><rect x="26" y="6" width="7" height="6" class="skf fp2"/>', 'STUDY 3 — module lamps · spec-annex coverage'),
