...
- **2026-09-12 — one wordmark.** The superior article is dropped from the ledger sites, and the atlas's name has ONE form everywhere: plain uppercase THE ALTITUDE ATLAS in the display face, nothing set apart. The user's reason, entire: no special treatment. The sheet-head PROJECT line and the title block's PROJECT value emitted `<sup class="art">the&nbsp;</sup>` ahead of the name until now, which made a CONDENSED wordmark distinct from the FULL one the rail head and the cover title carry; the distinction is gone with the superior, and what is left is one mark drawn at four sites at two sizes. `.project-mark` is unchanged and stays measured: 1.06em, because Eaglefeather at the resolved 700 caps at 0.651 of its em and DIN 2014 at 0.690, so the name's cap sits on the ledger line's — 7.94 px in a sheet head, 9.32 px in a title block. All three `PROJECT_MARK` twins moved in one step (`generator/chrome.mjs`, where the emitter is now simply the name wrapped in the span, `app/src/views.ts`, `app/prerender.ts`), so the site, the flat set and the artifact cannot drift apart. **The T9 article stays where it was born.** A sheet title is still "the MEASURED CITY" with the word as a 0.6em lowercase superior in the data face, the cover cards still draw it, and the title block's SHEET TITLE field still sets `the` inline through `.art-inline` — a ledger field is read as a value, not a headline, and neither rule was ever the wordmark's. Measured on the built `dist` at 1400: `/sheet/7` gives `.proj` "THE ALTITUDE ATLAS — DRAWING SET" and `.dsp` "THE ALTITUDE ATLAS" with no `sup.art` in either, `.sheet-title` with exactly one, and the cover's rail head and title unchanged; across `dist/`, `app/dist/`, `app/dist-artifact/` and the regenerated flat pages there are 522 `project-mark">THE ALTITUDE ATLAS` and not one superior before a `.project-mark`.
