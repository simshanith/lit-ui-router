 ...
 * `--title`, `--rail-title`, `--data`, `--prose`, `--hand`, `--code` — so five
 * candidate pairings and six INDEPENDENT knobs (sheet title, rail titles, data
 * face, data size, prose, hand) can be compared on the same copy, in both
 * themes, on both hosts. A pairing is a starting point, not a cage: picking one
 * resets the knobs it owns, and every knob then overrides it.
 *
...
export const ADOBE_FAMILIES = [
  ...
  },
  {
    css: 'source-serif-4',
    name: 'Source Serif 4',
    role: 'prose · the SOURCE SERIF 4 knob — the Google family answers on both hosts',
    woff2: '',
    kit: false,
  },
  {
...
export const PROSE_FACES = [
  ...
  },
  {
    ...
    google: 'Source+Serif+4:ital,wght@0,400;1,400',
    note: 'Slimbach’s open text serif — the same family on BOTH hosts, so the site and the artifact agree',
  },
  {
    id: 'minion',
    label: 'MINION PRO',
...
export class AtlasSpecimen extends LitElement {
  ...
  constructor() {
    ...
    this.pairing = 1;
    this.sheetTitle = 0;
    this.railMatch = false;
    this.hand = 0;
    this.dataFace = dataFaceIndex(PAIRINGS[1]?.dataDefault ?? 'din');
    this.prose = 0;
  ...
  #pick(index: number): void {
    this.pairing = index;
    this.dataFace = dataFaceIndex(PAIRINGS[index]?.dataDefault ?? 'din');
