...
const litZ0 = 96, litTop = litZ0 + B(1).courses * CRS;                 // 4 courses
const navZ0 = 140;                                                     // 1 course
const mbxZ0 = 252;                                                     // 2 courses, on top of brick 1
...
const clientBricks = [
  ...
  brick(OX, OY, 0, 120, 1, 1, B(3).courses, { z0: mbxZ0 }),
...
const SCHED = [
  ...
  [
    ...
    '    this.plugin(servicesPlugin) · this.stateRegistry.decorator(‘views’, litViewsBuilder) · urlService.listen()/sync()   [src/core.ts]',
  ],
  [
