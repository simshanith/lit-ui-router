...
<p><strong>REV F — the whole cabinet, one ref, and a district changed places.</strong> Every plate in <code>diagrams/data/</code> was re-counted at ${PLATE.ref} @ ${PLATE.sha} in one pass, and this deploy barely moved: the same ${fmt(PLATE.totals.files)} files, the same ${all.length} districts, still no orphans, still ${MB(PLATE.totals.gzBytes)} on the wire — 1,686 gzipped bytes more than rev E, on a four-megabyte deploy. Only the three documentation districts changed at all — html pages, page chunks and the VitePress framework each gained a fraction of a percent as the guides grew — and that fraction was enough to settle the closest race on the sheet. The HTML pages have overtaken Inter: ${fmt(PAGES.gz)} gz against ${fmt(INTER.gz)}, a ${LETTER_GAP}-byte lead where rev E had the fonts ahead by 504. The reading stands as it did, only sharper — the two tallest things this documentation site ships are prose and typography, and the corpora still tower over both.</p>
