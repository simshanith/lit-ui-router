...
// ---- census: every workspace member, this repo at HEAD, recounted 2026-08-31 ----
// Same basis as sheets 3 + 4 rev C — authored .ts/.tsx/.js/.jsx/.mjs under each
// member's source dir, excluding *.d.ts, *.test-d.ts, fixtures/, dist/,
// node_modules/; sloc = scc 4.0.0's `Code` count — string-aware, so the interior
// of a template literal counts as code rather than as one line.  Sheet 7 differs
// on one point, deliberately: it does not throw test code away.  Every file
// matching *.{spec,test,cy}.* or living under specs/ test/ tests/ __tests__/
// cypress/ is counted into a SECOND series and drawn as that member's annex.
// Generator: census-city.mjs (this directory).
...
const M = [
  ...
  [1,  'lit-ui-router',            'pkg',  'line',     0,  20, 12, 1325, 15, 3757, 'the subject of this set · annex 2.8× the source'],
  ...
  [5,  'sample-app-shared',        'app',  'line',   570,  10, 36, 2132,  3,  309, 'the routes + views every sample app mounts'],
  [6,  'sample-app-lit-vanilla',   'app',  'line',   720,  10,  8,  404,  0,    0, 'the plain-lit demo'],
  [7,  'sample-app-lit-mobx',      'app',  'line',   720,  90,  8,  443,  0,    0, 'the mobx demo'],
  [8,  'sample-app-routes',        'app',  'line',   700, 150,  2,   51,  1,  185, 'the shared route table, server-side too'],
  [9,  'sample-app-lit-e2e',       'app',  'pr',     580, 150,  1,   57,  6,  477, 'cypress · drives the docs build'],
  // --- docs/ + examples/ — the shopfront ---------------------------------------
  [10, 'docs',                     'site', 'line',   660, 340,  8,  688,  1,  181, 'vitepress + the worker that serves it'],
  [11, 'examples',                 'site', 'line',   660, 430, 10, 1387,  0,    0, 'stackblitz-ready copies, own lockfiles'],
  // --- tools/ — the instrument yard --------------------------------------------
  [12, '@tools/release',           'tool', 'halt',    20, 430, 44, 1871, 19, 1898, 'hosts published-diff — the one publish halt'],
  [13, '@tools/typedoc-plugin',    'tool', 'report', 230, 430,  5,  759,  0,    0, 'builds the API pages, gates nothing'],
  [14, '@tools/dts-backtest',      'tool', 'pr',       8, 350,  1,  291,  0,    0, 'one 291-line run.ts holds the TS 5.0 floor'],
  [15, '@tools/build_and_test',    'tool', 'report', 330, 430,  7, 1128,  3, 1163, 'the CI graph helper — and its error summary'],
  [16, '@tools/shared',            'tool', 'report',  20, 550, 10,  301,  5,  276, 'the library under the instruments'],
  [17, '@tools/workers-builds',    'tool', 'late',   220, 550,  2,  375,  1,  269, 'the docs deploy watch'],
  ...
  [28, '@tools/lint-elements',     'tool', 'pr',     380, 550,  1,  186,  0,    0, 'the shared custom-element lint lane (#655)'],
  [29, '@tools/warn-lanes',        'tool', 'report', 430, 530,  1,  214,  1,  280, 'the warning ratchet — reports a floor, never gates'],
  [30, '@tools/eslint-ts-parser',  'tool', 'report',  54, 350,  1,    1,  0,    0, 'a one-line parser shim — the smallest thing in the yard'],
];
...
${txt(58, SY + 58 + half * 17, `TOTAL — 30 members, 29 massed · ${TOT_SF} authored files · ${fmt(TOT_SL)} sloc · plus ${TOT_PF} spec files · ${fmt(TOT_PL)} sloc of annex · recounted 2026-08-31 (sloc = scc 4.0.0 Code)`, 'lbls')}`;

const svg = `<svg viewBox="0 0 1560 ${SY + 104 + half * 17}" role="img" aria-label="An isometric census city of the whole lit-ui-router workspace, drawn in four dashed districts. Every workspace member is a block massed by its own measured source — footprint side proportional to the square root of its authored lines, height three pixels per authored file — and every member that has tests carries a hatched annex beside it, massed the same way from its spec files. Upper left is the packages district, the product: lit-ui-router and ui-router-server each stand beside an annex with a larger footprint than the building it guards. To the right is the apps district, where sample-app-shared is the broadest source block on the sheet at thirty-six files and 2,132 lines. Below it sits the shopfront of docs and examples, and lower left the instrument yard of nineteen tools, dominated by the tall red release block — forty-four files and 1,871 lines, the only structure that can halt a publish. Gate severity is carried in colour, never in height: solid red halts a publish, red hatch stops the pull request line, accent hatch gates a later stage, faint blocks never gate at all. Roads run between the districts and carry real edges from the repository: solid roads where one member builds into another, dashed soft roads where tests exercise the code they cover, and an accent road that leaves both the source block and its spec annex together, because typecheck reads both. A structure schedule below lists all thirty members with exact file and line counts, their annex, and their gate tier.">
${defs(P)}

<rect x="40" y="26" width="520" height="42" class="skf fnone"/>
${txt(52, 43, 'THE CENSUS CITY — WHO IS ACTUALLY HERE', 'lbls')}
${txt(52, 58, '30 workspace members · 4 districts · every road below is a real edge in the repo', 'lblf')}
...
${[
  ...
  ['skr', 'fp', 'hr', 'stops the PR line — 9 · 14 · 19 · 23 · 26 · 28', 'lblr'],
  ['ska', 'fp', 'ha', 'gates a later stage — 17', 'lbla'],
  ['skf', 'fp2', null, 'never gates — 13 · 15 · 16 · 18 · 22 · 24 · 25 · 29 · 30', 'lbls'],
...
${txt(772, 123, '4 published packages · 25 files · 2,704 sloc', 'lblf')}
${txt(772, 135, 'every one carries a bigger annex than itself', 'lblf')}
<line x1="766" y1="126" x2="742" y2="168" class="skf"/>

${txt(1540, 300, 'sample-app-shared — 36f · 2,132 sloc', 'lblb', 'end')}
...
${txt(1540, 393, '5 members · 55 files · 3,087 sloc', 'lblf', 'end')}
...
${txt(1014, 669, '2 members · 18 files · 2,075 sloc', 'lblf')}
<line x1="1008" y1="652" x2="986" y2="636" class="skf"/>

${txt(60, 560, 'tools/ — THE INSTRUMENT YARD', 'lblb')}
${txt(60, 573, '19 members · 93 files · 5,791 sloc', 'lblf')}
...
${txt(60, 132, '12 authored files · 1,325 sloc', 'lblf')}
${txt(60, 144, 'its annex — 15 files · 3,757 sloc, 2.8× the source', 'lblf')}
...
export const sheet7 = {
  ...
  notes: `
<p><strong>Method — one basis, two series.</strong> Recounted from this repo's authored source at HEAD on 2026-08-31, the same basis as sheets 3 and 4: <code>.ts/.tsx/.js/.jsx/.mjs</code> under each member's source directory, excluding <code>*.d.ts</code>, <code>*.test-d.ts</code>, fixtures, <code>dist/</code> and <code>node_modules/</code>; <em>sloc</em> is <code>scc</code> 4.0.0's <code>Code</code> count — string-aware, so the interior of a template literal counts as code rather than as one line. Sheet 3 throws test code away; this sheet does not — anything matching <code>*.{spec,test,cy}.*</code> or living under <code>specs/ test/ tests/ __tests__/ cypress/</code> is counted into a second series and drawn as that member's annex. 30 members: 191 authored files and 13,657 sloc of source, plus 81 files and 11,927 sloc of spec. The packages district reconciles exactly with sheet 3's source slab — 25 files, 2,704 lines.</p>
<p><strong>Every shipped building still has an annex bigger than itself.</strong> <code>lit-ui-router</code>: 1,325 authored lines against 3,757 of spec (2.8×). <code>ui-router-server</code>: 1,141 vs 2,174 (1.9×). <code>lit-ui-router-mobx</code>: 133 vs 380 (2.9×). <code>navigation-location-plugin</code> is one 105-line file with seven spec files over it (3.9×). The test budget lives exactly where the repo says it should — in <code>packages/*</code>, and nowhere else at that ratio.</p>
