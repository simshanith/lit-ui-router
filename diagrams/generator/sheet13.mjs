...
// ---- census: file age × churn from git history, this repo at HEAD 2026-08-31 ----
...
// touch count — from ONE `git log -M --name-status` pass over all 516 commits,
...
// 2026-06: 3 · 2026-07: 224 · 2026-08: 144).  So the age bands are not round
...
//   SEASON III THE SUMMER    born 2026-07-04 →  228 files (84% of the city)
// Churn tiers, from the per-block touches-per-file distribution (the clean gap
// between 4.6 and 6.0 survives the recount; per-file median is still 2, from
// touch quartiles [1, 2, 4]): HOT ≥ 6 · COLD < 2.
// Sealed = idle > 180 days (the idle histogram is now empty from 61 to 180 days;
// the same 6 files sit beyond the gap, all Season II).

// ---- geometry: sheet 7 rev B's plan footprints, flattened -----------------------
// Same placements, same side = 1.6·√sloc rule (sloc as recounted for sheet 7 rev
// D, 2026-08-31 on scc 4.0.0's `Code` basis — reused as pure geometry so the two
// sheets reconcile by eye).  Weathering counts below are 2026-08-31 at HEAD:
// 272 files against sheet 7's 272 — the two universes now agree exactly.
...
const D = [
  [1, 'lit-ui-router', 'pkg', 0, 20, 1325, 3757, [12, [7, 0, 5], 85], [15, [1, 5, 9], 64], '2025-07-21', 44, 'ui-sref.ts×15', 0],
  [2, 'ui-router-server', 'pkg', 200, 20, 1141, 2174, [8, [0, 0, 8], 23], [12, [0, 0, 12], 22], '2026-07-14', 44, 'index.ts×4', 0],
  [3, 'lit-ui-router-mobx', 'pkg', 170, 130, 133, 380, [4, [0, 0, 4], 12], [4, [0, 0, 4], 10], '2026-07-04', 22, 'reaction-controller.ts×4', 0],
  [4, 'navigation-location-plugin', 'pkg', 260, 130, 105, 410, [1, [0, 1, 0], 10], [7, [0, 1, 6], 10], '2026-01-11', 22, 'index.ts×10', 0],
  [5, 'sample-app-shared', 'app', 570, 10, 2132, 309, [36, [8, 0, 28], 167], [3, [0, 0, 3], 3], '2025-07-21', 43, 'router.config.ts×14', 0],
  [6, 'sample-app-lit-vanilla', 'app', 720, 10, 404, 0, [8, [6, 0, 2], 48], null, '2025-07-21', 52, 'Compose.ts×10', 0],
  [7, 'sample-app-lit-mobx', 'app', 720, 90, 443, 0, [8, [0, 0, 8], 32], null, '2026-07-04', 52, 'Compose.ts×6', 0],
  [8, 'sample-app-routes', 'app', 700, 150, 51, 185, [2, [0, 0, 2], 3], [1, [0, 0, 1], 2], '2026-07-14', 47, 'routes.ts×2', 0],
  [9, 'sample-app-lit-e2e', 'app', 580, 150, 57, 477, [1, [1, 0, 0], 7], [6, [1, 0, 5], 23], '2025-07-21', 15, 'sample_app.cy.js×8', 0],
  [10, 'docs', 'site', 660, 340, 688, 181, [8, [2, 1, 5], 60], [1, [0, 0, 1], 1], '2025-07-23', 30, 'config.ts×25', 0],
  [11, 'examples', 'site', 660, 430, 1387, 0, [10, [0, 6, 4], 37], null, '2026-01-01', 20, 'hellosolarsystem/main.ts×11', 3],
  [12, '@tools/release', 'tool', 20, 430, 1871, 1898, [44, [0, 0, 44], 83], [19, [0, 0, 19], 32], '2026-07-06', 43, 'check-pack.ts×9', 0],
  [13, '@tools/typedoc-plugin', 'tool', 230, 430, 759, 0, [5, [0, 4, 1], 15], null, '2026-01-09', 234, 'index.ts×8', 3],
  [14, '@tools/dts-backtest', 'tool', 8, 350, 291, 0, [1, [0, 0, 1], 6], null, '2026-07-06', 17, 'run.ts×6', 0],
  [15, '@tools/build_and_test', 'tool', 330, 430, 1128, 1163, [7, [0, 0, 7], 19], [3, [0, 0, 3], 7], '2026-07-07', 37, 'upload-bundle-stats.ts×4', 0],
  [16, '@tools/shared', 'tool', 20, 550, 301, 276, [10, [0, 0, 10], 21], [5, [0, 0, 5], 6], '2026-07-06', 37, 'workspace.ts×9', 0],
  [17, '@tools/workers-builds', 'tool', 220, 550, 375, 269, [2, [0, 0, 2], 6], [1, [0, 0, 1], 7], '2026-07-11', 17, 'wb-triggers.test.ts×7', 0],
  [18, '@tools/bundle-probe', 'tool', 330, 550, 236, 0, [4, [0, 0, 4], 9], null, '2026-07-18', 22, 'upload-entry-bundles.ts×4', 0],
  [19, '@tools/compat-guards', 'tool', 130, 550, 189, 112, [7, [0, 0, 7], 9], [1, [0, 0, 1], 1], '2026-08-09', 22, 'catalog.ts×2', 0],
  [20, '@tools/oxc-emit', 'tool', 230, 350, 100, 0, [3, [0, 0, 3], 6], null, '2026-07-17', 30, 'emit-dts.ts×2', 0],
  [21, '@tools/release-config', 'tool', 280, 350, 39, 0, [1, [0, 0, 1], 2], null, '2026-07-31', 22, 'release-it.js×2', 0],
  [22, '@tools/lit-template-lint', 'tool', 325, 350, 21, 0, [1, [0, 0, 1], 2], null, '2026-08-11', 0, 'lint-templates.ts×2', 0],
  [23, '@tools/lit-test-env', 'tool', 85, 350, 24, 0, [1, [0, 0, 1], 1], null, '2026-08-09', 22, 'setup.ts×1', 0],
  [24, '@tools/vue-check', 'tool', 370, 350, 25, 0, [1, [0, 0, 1], 3], null, '2026-07-09', 48, 'bin.ts×3', 0],
  [25, '@tools/lcov-rebase', 'tool', 415, 350, 23, 30, [2, [0, 0, 2], 2], [1, [0, 0, 1], 1], '2026-07-29', 33, 'rebase-lcov.ts×1', 0],
  [26, '@tools/happy-dom', 'tool', 125, 350, 8, 26, [1, [0, 0, 1], 1], [1, [0, 0, 1], 1], '2026-07-19', 43, 'append.ts×1', 0],
  [27, '@tools/wintercg-globals', 'tool', 185, 350, 0, 0, null, null, '—', 0, '—', 0],
  // --- laid at rev C: the newest stone was cut the day it was surveyed ----------
  [28, '@tools/lint-elements', 'tool', 380, 550, 186, 0, [1, [0, 0, 1], 2], null, '2026-08-31', 0, 'lint-elements.ts×2', 0],
  [29, '@tools/warn-lanes', 'tool', 430, 530, 214, 280, [1, [0, 0, 1], 1], [1, [0, 0, 1], 1], '2026-08-31', 0, 'warn-lanes.core.test.ts×1', 0],
  [30, '@tools/eslint-ts-parser', 'tool', 54, 350, 1, 0, [1, [0, 0, 1], 1], null, '2026-08-16', 15, 'index.ts×1', 0],
];
...
${[
  ...
  [null, 'SEASON III · SUMMER · 07-04 on · 228f', 'sk'],
].map(([fill, label], i) => {
  const y = 256 + i * 24;
...
${txt(638, 482, 'I ×9.6 · II ×5.6 · III ×2.3', 'lbla')}
...
const COMMITS = { '2025-07': 42, '2026-01': 103, '2026-06': 3, '2026-07': 224, '2026-08': 144 };
const TOUCH = {
  pkg: { '2025-07': 20, '2026-01': 36, '2026-07': 132, '2026-08': 48 },
  app: { '2025-07': 39, '2026-01': 29, '2026-07': 173, '2026-08': 44 },
  site: { '2025-07': 10, '2026-01': 29, '2026-07': 40, '2026-08': 19 },
  tool: { '2025-07': 0, '2026-01': 5, '2026-07': 177, '2026-08': 63 },
};
...
${txt(58, SY + 58 + half * 17, `TOTAL — ${TOT_F} dated files (sheet 7 rev D counts the same 272) · ${fmt(TOT_T)} touches across 516 commits · seasons 26 / 18 / 228 · dated 2026-08-31`, 'lbls')}`;
...
${txt(1250, 700, 'docs/.vitepress/config.ts ×25', 'lblr')}
${txt(1250, 714, 'the most-weathered wall in the city —', 'lblf')}
${txt(1250, 726, '404 days old, repainted 25 times, idle 14d', 'lblf')}
<line x1="1246" y1="710" x2="1188" y2="702" class="skf"/>

${txt(1250, 780, 'examples/ — THE OLDEST UNTOUCHED STONE', 'lbls')}
${txt(1250, 794, 'two vite configs sealed 242 days,', 'lblf')}
${txt(1250, 806, 'helloworld/main.ts sealed 224 — all winter-built', 'lblf')}
<line x1="1246" y1="790" x2="1178" y2="800" class="skf"/>

${txt(560, 1078, '@tools/typedoc-plugin — SEALED WING', 'lbls')}
${txt(560, 1092, 'symbols/ untouched 234 days, since 2026-01-09', 'lblf')}
...
${txt(1520, 34, 'DATING — first-commit and last-touch per file from one pass over all 516 commits, renames followed', 'lbls', 'end')}
${txt(1520, 48, 'footprints reused from sheet 7 rev D as pure geometry (side = 1.6·√sloc) · weathering dated 2026-08-31', 'lblf', 'end')}
...
export const sheet13 = {
  num: 13, id: 'weathering', rev: 'C',
  title: 'THE WEATHERING MAP',
  sub: 'ALTITUDE t — the same city as sheet 7, surveyed in time · 272 files dated from 516 commits · three construction seasons, ten silent months · dated 2026-08-31 · REV B: drafting pass — the timeline rows now clear their own tallest bar, and no badge sits on a caption · REV C 2026-08-31: re-dated at TODAY = 2026-08-31 with the three new instruments in the universe; every idle figure below is two weeks older than rev B’s, which is clock, not neglect',
  scale: 'WORKSPACE × TIME',
  form: 'WEATHERING MAP',
  svg,
  caption: 'Every sheet so far has drawn the city as it stands; this one dates the stone. Same districts, same footprints as sheet 7 — but the fills now say when each file was first committed and the edges say how often it is touched. The history turns out to be three construction campaigns separated by ten silent months, and the two encodings disagree on purpose: 84% of the city is summer-built and barely weathered, while the port’s original masonry — 26 files from July 2025 — carries the hottest edges on the map.',
  notes: `
<p><strong>Method — one pass, renames followed.</strong> Every file in sheet 7's universe was dated from a single <code>git log -M --name-status</code> pass over all 516 commits, walked newest to oldest with rename chains followed backwards — equivalent to per-file <code>--follow</code> (spot-verified against it, including <code>tools/shared/workspace.ts</code>, which the plain path log mis-dates by a week) but one process instead of 272. Per file: first-commit date, last-touch date, touch count. Merge commits list no files under the default log, so a touch is a non-merge commit — the standard convention. Footprints and placements are sheet 7 rev B's, reused as pure geometry so the two sheets reconcile by eye; the weathering itself is dated 2026-08-31 at HEAD, where the city has 272 files — exactly the 191 source and 81 spec files sheet 7 rev D counts, so for the first time the two universes reconcile file for file.</p>
<p><strong>The bands are campaigns, not round numbers.</strong> The age histogram is not a slope, it is three spikes: files born around day 406 (July 2025), around day 224–242 (January 2026), and within the last 58 days — with <em>nothing</em> between them, because the repo has source births or touches in only four of its fourteen calendar months (commits in five: 42 · 103 · 3 · 224 · 144). So the map bins by season, where the distribution actually cuts: SEASON I, the port — 26 files that arrived 2025-07-21 with the upstream sample-app lineage; SEASON II, the winter works — 18 files including the typedoc plugin and the examples; SEASON III, the summer — 228 files, 84% of the city, none older than 2026-07-04. Churn tiers come from the per-block distribution the same way: touches-per-file still has a clean gap between 4.6 and 6.0, so HOT is ≥6; the per-file median is still 2, so COLD is below it. SEALED is idle &gt;180 days — the idle histogram is empty from 61 to 180, and exactly the same six files sit beyond the gap.</p>
<p><strong>The verdict: age and churn run opposite ways.</strong> Mean touches per file falls monotonically with youth — ×9.6 for the port cohort, ×5.6 for winter, ×2.3 for summer — and 105 of 272 files (39%) have been touched exactly once, ever: two weeks and eight new files later, that share has not moved a point. The port's masonry is not museum stone: 10 of <code>lit-ui-router</code>'s 27 files were touched in the last month, <code>ui-sref.ts</code> is at ×15 with the last chisel-mark dated 2026-08-16, and the two most-weathered walls in the city are the port-era <code>docs/.vitepress</code> pair — <code>config.ts</code> at ×25 and <code>vite.config.ts</code> at ×20. What survives from the first day is precisely what keeps being worked.</p>
<p><strong>What each district's weather says.</strong> The instrument yard — 5,791 sloc, the city's largest district by mass — logged <em>zero</em> touches before 2026 and is almost entirely summer stone at ×2.0 per file: infrastructure arrived late, fast, and mostly settled on the first cut. Its one winter building is also the map's sealed wing: the typedoc plugin's <code>symbols/</code> trio, untouched 234 days. The shopfront splits in two: <code>docs</code> runs the hottest multi-file block-average in the city (×7.5/f src) while <code>examples</code> holds the oldest untouched stone — two vite configs idle 242 days. And the smallest building on the map has the sharpest weather: <code>navigation-location-plugin</code>'s single January wall has been chiselled ten times, the highest per-file churn anywhere — matching its history as the extracted plugin that every routing change touches.</p>
<p><strong>REV C — two weeks of clock, three new stones, one new ruler.</strong> The map is re-dated at <code>TODAY = 2026-08-31</code>, so every idle figure is fourteen days larger than rev B's for reasons that are calendar, not neglect: <code>lit-ui-router</code>'s median idle reads 44 rather than 30 because nobody touched it, which is what an idle figure is for. The <em>bands</em> survived the recount intact — the touches-per-file gap is still between 4.6 and 6.0, the per-file median is still 2, and the same six files still sit beyond the 180-day seal, though the empty stretch is now 61–180 rather than 60–180. Footprints come from sheet 7 rev D, whose sloc basis changed to <code>scc</code> 4.0.0's <code>Code</code> count; that moves the geometry a little without touching a single date. Three new stones were laid: <code>@tools/lint-elements</code> and <code>@tools/warn-lanes</code> on 2026-08-31 and <code>@tools/eslint-ts-parser</code> on 2026-08-16 — all Season III, all cold, and the first two were cut the same day the map was surveyed, which is the youngest a wall on a weathering map can be.</p>
