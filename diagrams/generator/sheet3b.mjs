...
const ART_H = 770;
const SY = ART_H + 14;
// The right column stood 0.5px off the longest //# row in the left one: the two
// halves now split the band's spare measure evenly — 40px of gutter, 40px of
// right margin — instead of the whole slack piling up on the right.
...
${txt(430, 751, `format:check hashes ${fmt(row('examples#format:check').inputs)} files, lint ${fmt(row('examples#lint').inputs)} — the corpus from sheet 9, now as CI surface`, 'lblf')}
