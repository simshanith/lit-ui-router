...
try {
  ...
  const STATIONS = [
    { id: 'location', label: 'LOCATION', sub: 'history · hash · Navigation API', file: 'core.ts', line: 266, expect: 'this.urlService.listen();' },
    { id: 'core', label: '@uirouter/core', sub: 'state registry · match · go()', file: 'core.ts', line: 224, expect: 'export class UIRouterLit extends UIRouter {' },
    { id: 'hall', label: 'TRANSITION HALL', sub: 'onBefore | onStart · resolve | onSuccess', file: 'ui-view.ts', line: 230, expect: 'router.transitionService.onBefore({}, (trans) => {' },
    { id: 'view', label: '<ui-view>', sub: 'the viewport', file: 'ui-view.ts', line: 72, expect: 'export class UiView extends LitElement {' },
    { id: 'child', label: "<ui-view name='detail'>", sub: 'nested — stands on its parent', file: 'ui-view.ts', line: 176, expect: 'static seekParentView(candidate: Element): UiView | null {' },
    { id: 'render', label: 'LIT RENDER', sub: 'RoutedLitTemplate · html``', file: 'ui-view.ts', line: 379, expect: 'render(): Node | TemplateResult {' },
  ...
  const LEGS = [
    ...
    { id: 'render', from: 'view', to: 'render', kind: 'loop', carrier: 'requestUpdate() → render() → component({ router, resolves, transition })', short: 'state.component / template',
      ev: { file: 'ui-view.ts', line: 397, expect: 'return this.component({', span: 3 } },
    { id: 'commit', from: 'render', to: 'document', kind: 'loop', carrier: 'createRenderRoot(): this — the template lands in the light DOM', short: 'directives commit',
      ev: { file: 'ui-view.ts', line: 92, expect: 'createRenderRoot(): this {', span: 3 } },
    ...
    { id: 'nest', from: 'child', to: 'view', kind: 'event', carrier: 'ui-view-context (composed) — the parent adopts', short: 'ui-view-context',
      ev: { file: 'ui-view.ts', line: 178, expect: 'candidate.dispatchEvent(uiViewContextEvent);', span: 2 } },
  ...
  const WALK = [
    { title: 'AT REST — /people', legs: ['ctx', 'nest', 'target'],
      ...
      ev: [{ file: 'ui-view.ts', line: 191, expect: 'this.uiRouter = this.uiRouter || UIRouterLitElement.seekRouter(this);' },
        { file: 'ui-view.ts', line: 183, expect: 'this.parentView = this.constructor.seekParentView(this)!;' },
    ...
    { title: 'onBefore — CAN WE LEAVE?', legs: ['run'],
      ...
      ev: [{ file: 'ui-view.ts', line: 230, expect: 'router.transitionService.onBefore({}, (trans) => {', span: 3 },
        { file: 'ui-view.ts', line: 274, expect: 'if (trans.exiting().includes(state)) {', span: 3 }] },
    ...
    { title: 'RESOLVE', legs: ['run'],
      ...
      ev: [{ file: 'ui-view.ts', line: 386, expect: 'const resolvables = this.resolveContext', span: 3 },
        { file: 'ui-view.ts', line: 390, expect: '.filter((r) => r.resolved);' }] },
    { title: 'VIEWS ACTIVATE', legs: ['activate'],
      ...
      ev: [{ file: 'core.ts', line: 237, expect: "this.viewService._pluginapi._viewConfigFactory('lit', viewConfigFactory);" },
        { file: 'ui-view.ts', line: 221, expect: 'configUpdated: this._viewConfigUpdated.bind(this),' },
        { file: 'ui-view.ts', line: 132, expect: 'this.resolveContext = new ResolveContext(config.path);', span: 3 }] },
    { title: 'ONE LEVEL DOWN', legs: ['descend'],
      ...
      ev: [{ file: 'ui-view.ts', line: 213, expect: "const fqn = parentFqn ? parentFqn + '.' + name : name;" },
        { file: 'ui-view.ts', line: 242, expect: 'router.viewService.registerUIView(this._uiViewData),' }] },
    { title: 'LIT RENDER', legs: ['render'],
      ...
      ev: [{ file: 'ui-view.ts', line: 283, expect: 'requestUpdate(...args: Parameters<LitElement[\'requestUpdate\']>): void {', span: 3 },
        { file: 'ui-view.ts', line: 392, expect: 'const resolves = resolvables', span: 3 },
        { file: 'ui-view.ts', line: 397, expect: 'return this.component({', span: 3 }] },
    ...
    { title: 'onSuccess — THE VIEW HEARS IT', legs: ['run', 'activate'],
      ...
      ev: [{ file: 'ui-view.ts', line: 236, expect: 'router.transitionService.onSuccess({}, (trans) =>', span: 2 },
        { file: 'ui-view.ts', line: 294, expect: 'private _invokeUiOnParamsChangedHook($transition$: Transition) {', span: 2 },
        { file: 'ui-view.ts', line: 346, expect: 'instance.uiOnParamsChanged(newValues, $transition$);' }] },
    ...
    { title: 'THE URL — THE LOOP CLOSES', legs: ['sync'],
      ...
      ev: [{ file: 'core.ts', line: 265, expect: 'this.urlMatcherFactory.$get();', span: 3 }] },
