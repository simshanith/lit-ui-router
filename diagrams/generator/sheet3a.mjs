...
${txt(896, 281, 'turbo.json:351-365 — 11 dependsOn lanes under ci:pull_request', 'lblf')}
...
${clines(795, 452, [
  ...
  ['turbo.json:250-260', 'lblf'], ['same 4 legs, kept', 'lblf'],
...
${lf(40, 638, [
  ...
  '③ turbo //#lint:toml — executes the root script `mise run lint_toml` (package.json:32)',
...
${lf(560, 646, [
  ...
  '(turbo.json:263-267 · 280-286 · 291-297)',
...
const SCHED = [
  ...
  'C   turbo → mise — 7 root scripts (6 in ci) · crossing: script body `mise run …` · buys: TURBO CACHES MISE — inputs hash the task files + mise.lock · turbo.json:263-297',
...
${txt(1360, 62, `REV D whole-cabinet refresh — mise STILL unmoved at ${M.tasks} tasks / ${W.callSites} call sites · turbo ${T.definitions} definitions in ${T.files} files (97 in 18 at rev C) · ci 590→${CI.nodes} nodes, 176→${CI.real} real`, 'lblf', 'end')}
...
<p><strong>Two curiosities the census surfaced.</strong> One task in the whole machine is dead: <code>//tools/build_and_test:playwright_deps</code> (mise.toml:72-76) has no caller anywhere, superseded by <code>playwright_deps_engines</code>. And one umbrella exists twice: mise <code>lint_workflows</code> (a 4-leg <code>depends</code>, config.toml:131-135) and turbo <code>//#lint:workflows</code> (a virtual <code>with</code> node, turbo.json:250-260) are the same shape maintained by hand in both schedulers — the one place the two machines duplicate rather than delegate. The phantom shroud around turbo’s core — ${PHANTOM} of ${CI.nodes} nodes that exist only to carry hashes — is drawn as wall thickness here; sheet 12 punches it hole by hole.</p>`,
  key: [
