...
<p><strong>Rev C — off the plate.</strong> The hand-pasted constants are gone: rows, columns, cells, tallies, the overlay and the schedule are all read from <code>diagrams/data/census-plate.json</code> at draw time, and the sheet throws rather than draws if a pipeline or a fanned name it needs is missing. Re-surveyed at ${PLATE.ref} @ ${PLATE.sha}, the graph has grown again: <code>packages/eslint-plugin-lit-ui-router</code> is the fifth publishable package, taking the register from 30 fanned rows to ${PKGS} and the graph from 535 nodes / 165 real to ${CI.nodes} / ${CI.real}; edges 1,375 → ${fmt(CI.edges)}, real edges 117 → ${CI.realEdges}, phantom share 69% → ${phantomPct(CI)}%. There is also a ${COLS.length}th fanned column — <code>check:dev-split</code>, the dev-warning split guard, command-bearing in ${nm('check:dev-split').real} package and a placeholder in the other ${nm('check:dev-split').nodes - nm('check:dev-split').real} — which is the same story the eighteen told, one column wider. The ragged tail took the new package's three singletons (<code>lint:docs</code>, <code>lint:rules</code>, <code>test:oxlint</code>) and stands at ${TAIL.length}; the deepest chain is still ${CI.chain} rungs with ${CHAIN_REAL} real, but the longest all-real chain went back up to ${CI.realChain} <code>test</code> tasks — a new publishable package with a suite is exactly the sort of member that lengthens it.</p>`,
  key: [
