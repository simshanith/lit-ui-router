...
// ---------------------------------------------------------------------------
// DATA — from census-plate.mjs (`turbo run <pipeline> --dry=json`, turbo 2.10.11,
// worktree altitude-atlas, recounted 2026-08-31). Cell codes: R = command-bearing,
...
const COL_TOT = [[30, 0], [29, 5], [26, 5], [30, 4], [9, 4], [30, 26], [29, 5], [28, 2], [28, 1], [30, 30], [30, 30], [30, 17], [30, 4], [30, 2], [30, 1], [30, 4], [30, 0], [30, 0]];
...
const GROUPS = [
  ...
  ['TOOLS/ — INSTRUMENTS ×19', [
    ...
    ['@tools/eslint-ts-parser', 'pppp-RpppRRppppppp'],
    ['@tools/happy-dom', 'pppp-RpppRRRpppppp'],
    ['@tools/lcov-rebase', 'pppp-RpppRRRpppppp'],
    ['@tools/lint-elements', 'pppp-RpppRRppppppp'],
    ...
    ['@tools/warn-lanes', 'pppp-RpppRRRpppppp'],
...
const TAIL = [
  ...
  ['//#format:check:toml', 1], ['//#lint:actionlint', 1], ['//#lint:elements', 1],
  ['//#lint:markdown', 1], ['//#lint:package-json', 1], ['//#lint:root', 1],
  ['//#lint:shellcheck', 1], ['//#lint:templates', 1], ['//#lint:toml', 1],
  ['//#lint:workflows', 0], ['//#lint:zizmor', 1], ['//#typecheck:root', 1],
...
// the deepest chain in the ci graph: 13 nodes, 5 of them command-bearing
const CHAIN = [
  ['@tools/warn-lanes#build:types', 0],
...
const LEDGER = [
  ['ci', 535, 165, 1375, 117, '13 / 6', 'the PR gate — 30 packages × 18 fanned names'],
  ['ci:main', 567, 170, 1538, 118, '13 / 6', '+32 nodes over 4 names; PR graph folded in'],
  ['build', 113, 22, 268, 14, '10 / 3', 'the emit tier alone — four fifths placeholder'],
  ['lint', 66, 47, 125, 31, '9 / 4', 'the densest real graph here — 71% real'],
  ['pack:all', 53, 12, 102, 5, '7 / 3', 'one real task atop a 53-node cone'],
];
...
${lines(RX, CLY + CN * 22 + 26, [
  'Five of those thirteen run a command.',
  'The longest all-real chain is six',
  '— and it is six «test» tasks in a',
...
${txt(30, TY + 14, 'fifteen of them belong to the root package «//», which appears in no fanned column at all', 'lblf')}
${TAIL.map(([n, r], i) => {
...
const svg = `<svg viewBox="0 78 1160 ${H - 78}" role="img" aria-label="A punchcard register plate of the pull-request CI task graph. Rows are the thirty workspace packages plus the root, grouped into publishable packages, sample apps, docs and examples, tools, and root. Columns are the eighteen task names that fan across packages, ordered by pipeline stage: gather, emit, check, prove, roll-up. A filled accent card-hole marks a command-bearing task node; a faintly hatched hole marks a placeholder node that runs nothing; a red crossed hole marks the three task names that are one hundred percent placeholder by design — transit, ci:pull_request and ci. Of 535 nodes only 165 run a command and only 117 of 1375 dependency edges connect two real tasks. A ghosted four-column overlay to the right shows the thirty-two extra nodes the ci:main pipeline adds. Beside the plate, the deepest chain in the graph is drawn as a thirteen-rung ladder of which five rungs are real, and the twelve uncacheable task definitions in the repository are listed as a tier that never appears in this graph. Below, twenty-six singleton tasks form a ragged tail, and a structure schedule totals five pipelines.">
${defs(P)}
...
${txt(30, 122, 'one card-hole per (package, task) node turbo puts in the graph · turbo 2.10.11 · --dry=json', 'lbls')}
${txt(1130, 106, '535 NODES · 165 RUN A COMMAND', 'lblb', 'end')}
${txt(1130, 122, '1,375 EDGES · 117 JOIN TWO REAL TASKS', 'lbls', 'end')}
...
${lines(30, 222, [
  ...
  'the 30 packages. They exist only',
...
export const sheet12 = {
  num: 12, id: 'graph', rev: 'B',
  title: 'THE REGISTER PLATE',
  sub: 'ALTITUDE 3¼ — the monorepo as its CI reads it · every task node punched · turbo 2.10.11 · REV B: census refresh 2026-08-31 — three new members, three new rows, the phantom share held',
  scale: 'PR CI GRAPH',
  form: 'REGISTER PLATE',
  svg,
  caption: 'The pull-request graph is a near-rectangle: thirty packages against eighteen fanned task names, plus a ragged tail of twenty-six one-offs. Two thirds of the holes are unpunched — placeholder nodes that run nothing, minted so a ^self chain has somewhere to land an edge.',
  notes: `
<p><strong>Method.</strong> <code>turbo run ci --dry=json</code> and <code>turbo run ci:main --dry=json</code> from the repository worktree, turbo 2.10.11, recounted 2026-08-31; the same for <code>build</code>, <code>lint</code> and <code>pack:all</code> in the schedule. A node is one <code>(package, task)</code> pair turbo placed in the graph. A node is <em>command-bearing</em> when its resolved command is not the literal <code>&lt;NONEXISTENT&gt;</code> — turbo's marker for a task it had to invent because something depends on it in that package. An edge is <em>real</em> only when both of its ends are. No duration is drawn anywhere: wall-clock comparisons here are confounded by cache state and by the task counts themselves, so this plate measures shape and nothing else.</p>
...
<p><strong>The finding.</strong> 535 nodes; 165 run a command. 1,375 dependency edges; 117 join two real tasks. Of the eighteen fanned names, five carry a <code>^self</code> chain — <code>build</code>, <code>build:types</code>, <code>test</code>, <code>test:coverage</code>, <code>transit</code> — and a self-chain is what mints placeholders: turbo needs a node in <em>every</em> package to hang the chain on, whether or not that package has such a script. Three names are 100% placeholder in all 30 packages: <code>transit</code> (which has no implementation anywhere in the repo — it exists purely as an edge), and the two roll-ups <code>ci:pull_request</code> and <code>ci</code>. The plate's rightmost columns and its leftmost are, in the strictest sense, empty.</p>
<p><strong>Depth is mostly scaffolding too.</strong> The deepest chain is thirteen nodes and five of them run anything. The longest chain of consecutive real nodes is six — and it is six <code>test</code> tasks in a row, serialized by <code>^test</code> and nothing else: no task in that chain consumes an artifact from the one above it. That is the price of a self-chain used for ordering rather than for data flow, and it is what a plate makes visible that a node-and-arrow render never does.</p>
<p><strong>What the plate is not evidence for.</strong> Placeholders are cheap — turbo schedules and skips them, and the phantom share is not a runtime cost. The argument is about legibility: 69% of what a maintainer sees in <code>--graph</code> output is scaffolding, and the two ratios worth watching are the real-node share per column and the real-edge count. Note also the one tier that never appears here at all: the repo's twelve <code>cache:false</code> definitions — four writers, six persistent servers and two live-network readers — are all outside every <code>ci:*</code> graph by design. A cache hit on any of them would be wrong, and the gate depends on their read-only twins instead (<code>format:check</code> for <code>format</code>).</p>
<p><strong>The overlay.</strong> <code>ci:main</code> is not a different pipeline; it swallows <code>ci:pull_request</code> whole and adds 32 nodes across four names — <code>ci:main</code>, <code>test:engines</code>, <code>test:matrix</code>, <code>check:pack</code> — of which five are command-bearing. The main-branch graph is 6% larger and buys three engine tests, one d.ts back-test and one pack check.</p>
