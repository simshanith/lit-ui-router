...
// census-couplings.json supplies this repo's own peer ranges (text read of the
// archive's package.json files, catalogs resolved) — the gates were hand-typed
// until rev E.
...
const MOBX_GATE = peerRange('lit-ui-router-mobx', 'lit-ui-router');
...
const ADAPTERS = [
  ...
  brick('lit-ui-router', CORE_GATE, 'peer', 'this'),
];
const INSTRUMENTS = [
...
${txt(1310, 558, `this repo's gates: census-couplings.json · ${COUPLINGS.ref} @ ${COUPLINGS.sha}`, 'lblf', 'end')}
...
export const sheet4 = {
  ...
  sub: `ALTITUDE 4 — one core, four living adapters, four dormant instruments · REV C: every version and publish date imported from census-npm.json (${REGISTRY}), core and this repo's mass from census-bricks.json · REV D: whole-cabinet refresh — the lint plugin's schedule line now prints BOTH its npm version and its in-repo version, which the rc.2 release pulled apart · REV E 2026-09-04: this repo's gates — the shared core gate and mobx's own on lit-ui-router — now read census-couplings.json instead of two hand-typed ranges, and the build throws if the companions ever stop sharing one`,
