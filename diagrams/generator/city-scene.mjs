...
(function () {
  ...
  function boot(THREE) {
    ...
    function mass(x, z, s, h, tier, lineMat) {
      var geo = new THREE.BoxGeometry(s, h, s);
      var px = x + s / 2 - cx, pz = z + s / 2 - cz;
      if (D.tiers[tier].f > 0) {
        var m = mats[tier];
        var mesh = new THREE.Mesh(geo, [m[1], m[2], m[0], m[0], m[2], m[1]]);
    ...
    rows.forEach(function (b) {
      mass(b.x, b.y, b.s, b.h, b.tier, b.tier === 'off' ? lines.off : lines.src);
      if (b.sa) mass(b.ax, b.ay, b.sa, b.ha, 'annex', lines.annex);
    });
    ...
    var pending = false;
    function draw() { pending = false; place(); renderer.render(scene, camera); }
    ...
    var dragging = false, engaged = false, lastX = 0;
    stage.addEventListener('pointerdown', function (e) {
      tween = null; dragging = true; engaged = true; lastX = e.clientX;
    ...
    stage.addEventListener('pointerleave', function () { engaged = false; });
    ...
    if (hint) hint.textContent = 'DRAG TO ORBIT · RELEASE SNAPS TO THE NEAREST DIAGONAL · SCROLL TO ZOOM · DOUBLE-CLICK RESETS';
