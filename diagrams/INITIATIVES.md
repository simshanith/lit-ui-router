...
`diagrams/data/<probe>.json`; sheets import the plate and render figures and provenance
...
The landing-by-landing narrative is NOT kept here. `diagrams/HISTORY.md` is the verbatim
...
| I4 | Snapshot-import sheet refactor | sheets read `diagrams/data/*.json`; generated basis lines |
...
- The atlas is LIVE at <https://atlas.lit-ui-router.dev/> — a Cloudflare Pages project
  ...
  cycle from `diagrams/`: `node generator/build.mjs .` → `npm --prefix app run build` →
  `node generator/stage-site.mjs` → `wrangler pages deploy dist --project-name
  altitude-atlas`. **This branch never merges to main** — it is the atlas's permanent home.
