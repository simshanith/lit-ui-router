...
const mymessagesState = {
  ...
  dsr: {
    default: 'mymessages.messagelist',
...
function againable<T>(fn: () => T) {
  return function $again(): Promise<T> & { $again: () => Promise<T> } {
    return Object.assign(Promise.resolve(fn()), {
      $again,
    });
  }
}
...
const messageListState = {
  ...
  resolve: [
    ...
    // The list of message for the folder are fetched from the Messages service
    {
      token: 'messages',
      deps: ['folder'],
      resolveFn: (folder) => againable(() => MessagesStorage.byFolder(folder))(),
    }
  ],
  views: {
