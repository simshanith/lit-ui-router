...
const prefixSubject = (prefix: string, message: Message) => prefix + message.subject;
const makeResponseMsg = (prefix: string, msg: Message) => ({ from: msg.to, to: msg.from, subject: prefixSubject(prefix, msg), body: quoteMessage(msg)});
...
export class MessageElement extends LitElement {
  ...
  get actions() {
    return this.folder.actions.reduce((obj, action)=> {
      obj[action] = true;
      return obj;
  ...
  render() {
    ...
    return html`<div class="message">
      <div class="header">
        ...
        <div class="line2">
          <div>
            ${this.actions.edit ? html`<button class="btn btn-primary" @click=${this.editDraft}><i class="fa fa-pencil"></i> <span>Edit Draft</span></button>` : null}
            ${this.actions.reply ? html`<button class="btn btn-primary" @click=${this.reply}><i class="fa fa-reply"></i> <span>Reply</span></button>` : null}
            ${this.actions.forward ? html`<button class="btn btn-primary" @click=${this.forward}><i class="fa fa-forward"></i> <span>Forward</span></button>` : null}
            ${this.actions.delete ? html`<button class="btn btn-primary" @click=${this.removeMessage}><i class="fa fa-close"></i> <span>Delete</span></button>` : null}
          </div>
        </div>
      </div>
