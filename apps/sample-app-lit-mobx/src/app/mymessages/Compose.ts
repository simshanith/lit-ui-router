...
export class Compose extends LitElement {
  ...
  // Otherwise an unchanged param is not a new draft — the reaction fires again
   ...
   *   reaction only fires when the `message` param structurally CHANGES, and
   *   a fresh New Message leaves it `{}` — same param, no reset.
   * - drop the DSR memory, or returning to Messages reopens this state.
   *
   * Deliberately NOT part of `uiCanExit` or the reaction: the point is
