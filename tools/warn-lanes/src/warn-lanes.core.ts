...
//      CI run summary parses back out into its always-on overview.
//
// The marker exists because a warn-only lane is invisible to everything else
// we have: turbo's run summary carries only startTime/endTime/exitCode per
// task — a task with 36 warnings and one with zero are byte-identical in that
// JSON. So the state has to be *asserted* by the lane, and the task log is the
// one channel turbo replays verbatim on a cache hit.
//
