...
import { isBoundedRange } from './ranges.ts';

export interface Guard {
  /**
   * Catalog range for a dep; fails when the catalog or the dep is absent, or
   * when the range names no bound. Rejecting it here rather than letting a
      ...
      if (!isBoundedRange(range)) {
        fail(
          `${catalog} ${dep} range "${range}" names no bound — it is either ` +
            'unreadable, or one of the spellings semver reads as `*` (`||`, ' +
